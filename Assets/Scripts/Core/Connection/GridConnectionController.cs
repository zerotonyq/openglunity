...
namespace Core.Connection
{
    ...
    public class GridConnectionController : MonoBehaviour
    {
        ...
        public void DeleteConnectableAndMove(Vector2Int position, Vector2Int centerPosition)
        {
            //replace with more elegant construction
            if (position == Vector2Int.zero)
                return;
            

            var deleteDirection = position - centerPosition;
            ...
            Vector2Int currentPosition = position + deleteDirection;

            Debug.Log(currentPosition + " CURRENT POSITION");
            while (connectors.TryGetValue(currentPosition, out Connector currentConnector))
            {
                var previousPosition = currentPosition - deleteDirection;
                
                Debug.Log("try delete on position " + currentPosition);
                
                if(!currentConnector.HasConnection)
                    continue;
                
                currentConnector.Connectable.transform.position =
                    new Vector3(previousPosition.x, previousPosition.y, 0);
                
                currentPosition += deleteDirection;
            }
        }

        public List<KeyValuePair<Vector2Int, Connector>> GetNearConnectors(Vector2Int position)
        {
            List<KeyValuePair<Vector2Int, Connector>> nearConnectors = new();
            for (int y = -1; y <= 1; y++)
            {
                for (int x = -1; x <= 1; x++)
                {
                    ...
                    if (!connectors.TryGetValue(currentPosition, out Connector connector))
                        continue;

                    nearConnectors.Add(new KeyValuePair<Vector2Int, Connector>(currentPosition, connector));
                }
            }

            return nearConnectors;
        }
    }
}